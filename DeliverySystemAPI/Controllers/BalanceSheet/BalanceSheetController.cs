...
namespace DeliverySystemAPI.Controllers.BalanceSheet
{
    public class BalanceSheetController : ControllerBase
    {
        private IConfiguration Configuration;
        public BalanceSheetController(IConfiguration configuration)
        {
        ...
        public IActionResult Get_City(String Business_Name)
        {
            try
            {
                ...
                string connection = Configuration.GetConnectionString(balanceSheetModel.DB_Name);
                SqlConnection con = new SqlConnection(connection);
                con.Open();
                SqlCommand get_city_cmd = new SqlCommand("SELECT ROW_NUMBER() OVER (ORDER BY C_ID DESC) AS  SrNo,BalanceSheet_ID,Cashin,Kpay,DeliMen,Total_Amount,ToGet,ToPay,GenerealExpense,Remark,CreateDate From BalanceSheet Where Business_Name=@business_name And IsDeleted=@is_deleted Order By BalanceSheet_ID DESC", con);
        ...
        [HttpPost]
        [Route("api/balancesheet")]
        public IActionResult Create_City([FromBody] CityModel balanceSheetModel)
        {
            string connection = Configuration.GetConnectionString(balanceSheetModel.DB_Name);
            try
            {
                ...
                get_city_cmd.Parameters.AddWithValue("@c_name", balanceSheetModel.C_Name);
                get_city_cmd.Parameters.AddWithValue("@business_name", balanceSheetModel.Business_Name);
                ...
                if (reader.HasRows)
                {
                    reader.Close();
                    GC.Collect();
                    return StatusCode(StatusCodes.Status409Conflict, balanceSheetModel.C_Name + " City Already Exist");
                }
                else
                {
                    reader.Close();
                    ...
                    create_city_cmd.Parameters.AddWithValue("@c_name", balanceSheetModel.C_Name);
                    create_city_cmd.Parameters.AddWithValue("@business_name", balanceSheetModel.Business_Name);
        ...
        [HttpPut]
        [Route("api/balancesheet")]
        public IActionResult Update_City([FromBody] CityModel balanceSheetModel)
        {
            try
            {
                string connection = Configuration.GetConnectionString(balanceSheetModel.DB_Name);
                ...
                else
                {
                    ...
                    update_city_cmd.Parameters.AddWithValue("@c_id", balanceSheetModel.C_ID);
                    update_city_cmd.Parameters.AddWithValue("@c_name", balanceSheetModel.C_Name);
                    update_city_cmd.Parameters.AddWithValue("@update_date", DateTime.Now);
                    update_city_cmd.Parameters.AddWithValue("@business_name", balanceSheetModel.Business_Name);
